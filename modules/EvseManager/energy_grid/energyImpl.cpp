        ...
        if (mod->config.charge_mode == "DC") {
            ...
            if (value.limits_root_side.has_value() && value.limits_root_side.value().total_power_W.has_value()) {
                ...
                if (last_enforced_limits.total_power_W != watt_leave_side ||
                    ...
                    types::iso15118_charger::DcEvseMaximumLimits evseMaxLimits;
                    ...
                    if (target_voltage > 10) {
                        ...
                        if (actual_voltage > 10) {
                            evseMaxLimits.evse_maximum_current_limit =
                        ...
                        evseMaxLimits.evse_maximum_current_limit = powersupply_capabilities.max_export_current_A;
                    }

                    if (evseMaxLimits.evse_maximum_current_limit > powersupply_capabilities.max_export_current_A)
                        evseMaxLimits.evse_maximum_current_limit = powersupply_capabilities.max_export_current_A;

                    if (powersupply_capabilities.max_import_current_A.has_value() &&
                        evseMaxLimits.evse_maximum_current_limit < -powersupply_capabilities.max_import_current_A.value())
                        evseMaxLimits.evse_maximum_current_limit = -powersupply_capabilities.max_import_current_A.value();

                    // now evseMaxLimits.evse_maximum_current_limit is between
                    // -max_import_current_A ... +max_export_current_A

                    evseMaxLimits.evse_maximum_power_limit = value.limits_root_side.value().total_power_W.value();
                    if (evseMaxLimits.evse_maximum_power_limit > powersupply_capabilities.max_export_power_W)
                        evseMaxLimits.evse_maximum_power_limit = powersupply_capabilities.max_export_power_W;

                    if (powersupply_capabilities.max_import_power_W.has_value() &&
                        evseMaxLimits.evse_maximum_power_limit < -powersupply_capabilities.max_import_power_W.value())
                        evseMaxLimits.evse_maximum_power_limit = -powersupply_capabilities.max_import_power_W.value();

                    // now evseMaxLimits.evse_maximum_power_limit is between
                    // -max_import_power_W ... +max_export_power_W

                    evseMaxLimits.evse_maximum_voltage_limit = powersupply_capabilities.max_export_voltage_V;
                    ...
                    if (evseMaxLimits.evse_maximum_power_limit < 0 || evseMaxLimits.evse_maximum_current_limit < 0) {
                        // we are exporting power back to the grid
                        if (mod->config.hack_allow_bpt_with_iso2) {
                            if (evseMaxLimits.evse_maximum_power_limit < 0) {
                                evseMaxLimits.evse_maximum_power_limit = -evseMaxLimits.evse_maximum_power_limit;
                                mod->is_actually_exporting_to_grid = true;
                            } else {
                                mod->is_actually_exporting_to_grid = false;
                            }

                            if (evseMaxLimits.evse_maximum_current_limit < 0) {
                                evseMaxLimits.evse_maximum_current_limit = -evseMaxLimits.evse_maximum_current_limit;
                                mod->is_actually_exporting_to_grid = true;
                            } else {
                                mod->is_actually_exporting_to_grid = false;
                            }
                            ...
                            evseMaxLimits.evse_maximum_power_limit = 0.;
                            evseMaxLimits.evse_maximum_current_limit = 0.;
                        }
                    } else {
                        mod->is_actually_exporting_to_grid = false;
                    }
                    ...
                    session_log.evse(
                        ...
                        fmt::format("Change HLC Limits: {}W/{}A, target_voltage {}, actual_voltage {}, hack_bpt {}",
                                    evseMaxLimits.evse_maximum_power_limit, evseMaxLimits.evse_maximum_current_limit,
